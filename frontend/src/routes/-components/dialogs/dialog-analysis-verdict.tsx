...
import { TrendingUpIcon, LightbulbIcon, CheckCircle2Icon, AlertCircleIcon } from "lucide-react";
    ...
    <div className="flex flex-col gap-2">
      ...
      <ul className="flex flex-col gap-1.5 pl-1">
        {items.map((item, i) => (
          ...
          <li key={i} className="flex items-start gap-2 text-sm text-(--sea-ink)">
...
function VerdictResult({ data }: { data: TVerdictResponse }) {
  return (
    <div className="flex flex-col gap-4 overflow-y-auto max-h-[60vh] pr-1">
      ...
      <SummarySection totalAmount={data.totalAmount} byCategory={data.byCategory} />
      <div className="h-px bg-(--line)" />
      <BulletList
        title="Insights"
        items={data.insights}
...
const DialogAnalysisVerdict = React.forwardRef<ModalHandle, {}>((_, outerRef) => {
